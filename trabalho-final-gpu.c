...
void seq_differentiate_X(double* X_src, double* X_dst, double drag_coeff, double drag_coeff_a, double drag_coeff_b, 
    double drag_coeff_vc, double drag_coeff_vs, double magnus_coeff, double gravity, int use_extra_forces) {
...
void seq_calculate(int result_size, double* results, int n_bytes, int use_extra_forces,
    double velocity, double timestep, double gravity, double theta_center, double theta_std_dev, double phi_center, 
    double phi_std_dev, int dim_to_stop, double value_to_stop, bool stop_on_lesser, double drag_coeff,
    double drag_coeff_a, double drag_coeff_b, double drag_coeff_vc, double drag_coeff_vs,
    double object_radius, double object_angular_velocity, double proportionality_constant,
    ...
    for (int k = 0; k < iterations; k++) {
        std::normal_distribution<double> theta_distribution(theta_center, theta_std_dev);
        std::normal_distribution<double> phi_distribution(phi_center, phi_std_dev);

        double theta = (theta_distribution(generator)) * (PI / 180.0);
        double phi = (phi_distribution(generator)) * (PI / 180.0);
        ...
        for (int i = 0; i < 1000000; i++) {
            //X' = (vx, vy, vz, dvx, dvy, dvz)
            seq_differentiate_X(X, X_prime, drag_coeff, drag_coeff_a, drag_coeff_b, drag_coeff_vc, drag_coeff_vs, magnus_coeff, gravity, use_extra_forces);
            ...
            //X'e(n+1) armazenado em Xe(n+1)
            seq_differentiate_X(X_1_E, X_1_E, drag_coeff, drag_coeff_a, drag_coeff_b, drag_coeff_vc, drag_coeff_vs, magnus_coeff, gravity, use_extra_forces);
            ...
            if ((((X[dim_to_stop] - value_to_stop) < 0) && stop_on_lesser) || (((X[dim_to_stop] - value_to_stop) > 0) && !stop_on_lesser)) {
                ...
                results[k * result_size + 0] = X[0] + ((timestep * ((X_prime[0] + X_1_E[0]) / 2.0))) * (proportion - 1);
                results[k * result_size + 1] = X[1] + ((timestep * ((X_prime[1] + X_1_E[1]) / 2.0))) * (proportion - 1);
                results[k * result_size + 2] = X[2] + ((timestep * ((X_prime[2] + X_1_E[2]) / 2.0))) * (proportion - 1);
                results[k * result_size + 3] = X[3] + ((timestep * ((X_prime[3] + X_1_E[3]) / 2.0))) * (proportion - 1);
                results[k * result_size + 4] = X[4] + ((timestep * ((X_prime[4] + X_1_E[4]) / 2.0))) * (proportion - 1);
                results[k * result_size + 5] = X[5] + ((timestep * ((X_prime[5] + X_1_E[5]) / 2.0))) * (proportion - 1);

                double stoptime = ((i-1) * timestep + (proportion) * timestep);
                results[k * result_size + 6] = stoptime;
...
int main(int argc, char** argv) {
    if (argc < 5) {
        printf("Usage:\n  <file> <n_blocos> <n_threads> <timestep> <verbose? [0 | 1]> <measure sequential? [0 | 1]>\n");
    ...
    bool should_measure_seq = (argc > 5) ? atoi(argv[5]) : false;
    double initial_velocity = (argc > 6) ? atof(argv[6]) : 25.0;
    double theta = (argc > 7) ? atof(argv[7]) : 15;
    double theta_std_dev = (argc > 8) ? atof(argv[8]) : 3;
    double phi = (argc > 9) ? atof(argv[9]) : 10;
    double phi_std_dev = (argc > 10) ? atof(argv[10]) : 3;
    int dim_to_stop = (argc > 11) ? atoi(argv[11]) : 1;
    double value_to_stop = (argc > 12) ? atoi(argv[12]) : 20;
    bool stop_on_lesser = (argc > 13) ? atoi(argv[13]) : false;
    int use_extra_forces = (argc > 14) ? atoi(argv[14]) : 1;
    ...
    if (should_measure_seq) {
        double* seq_results = (double*) malloc(n_bytes);

        GET_TIME(inicio);
        seq_calculate(result_size, seq_results, n_bytes, use_extra_forces, initial_velocity, timestep, gravity,
            theta, theta_std_dev, phi, phi_std_dev, dim_to_stop, value_to_stop, stop_on_lesser,
            drag_coeff, drag_coeff_a, drag_coeff_b, drag_coeff_vc, drag_coeff_vs,
            object_radius, object_angular_velocity, proportionality_constant, n_blocos * n_threads);
        GET_TIME(fim);
        ...
        for (int i = 0; i < n_blocos * n_threads; i++) {
            t_acc += seq_results[i * result_size + 6];
            x_acc += seq_results[i * result_size + 0];
            y_acc += seq_results[i * result_size + 1];
            z_acc += seq_results[i * result_size + 2];

            if (seq_results[i * result_size + 2] > 0.0   && seq_results[i * result_size + 2] < 2.44 &&
                seq_results[i * result_size + 2] > -3.66 && seq_results[i * result_size + 2] < 3.66) {
                goals++;
            }
        }
        ...
        printf("DURATION: \n");
        printf("  Sequential duration = %f seg \n", tempo_seq);
    }
